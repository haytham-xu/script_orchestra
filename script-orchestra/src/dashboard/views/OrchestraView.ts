...
// Status checks: each entry describes one GET call and how to derive running=bool.
interface StatusCheck {
  key: string   // matches TOOLS key (or a sub-key like 'download-jm')
  url: string
  isRunning: (data: any) => boolean
}
    ...
    async function pollStatus() {
      const results = await Promise.allSettled(
        STATUS_CHECKS.map(async (check) => {
          ...
          return check.isRunning(data) ? check.key : null
        })
      )
      const keys = results
