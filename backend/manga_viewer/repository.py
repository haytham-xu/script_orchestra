...
class Repository:
            ...
            for a_file_name in files:
                ...
                if lower.endswith(config.IMAGE_EXTS) or lower.endswith(config.VIDEO_EXTS):
                    ...
                    try:
                        ...
                        total_files += 1
                        # # 构造URL
                        # rel_for_url = a_file_path.lstrip(os.sep)
                        # from urllib.parse import quote
    ...
    def get_files_url_list(folder_path: str):
        ...
        # for fname in natsorted(root_files):
        ...
        # for dname in natsorted(subdirs):
        for dname in flex_natsort(subdirs):
            ...
            try:
                ...
                continue
            sub_files = [sf for sf in sub_entries if os.path.isfile(os.path.join(subdir_path, sf))]
            # for sf in natsorted(sub_files):
    ...
    def refresh_index():
        ...
        path_id_map = {folder_instance.path: folder_id for folder_id, folder_instance in Repository.manga_index.folders.items()}
        ...
        for to_scan_path in Repository.scan_paths:
            ...
            for a_name in os.listdir(to_scan_path):
                ...
                if os.path.isdir(a_folder_path) and a_folder_path not in Repository.ignore_scan_paths:
    ...
    def update_folder(folder_id: str, new_folder_instance: Folder):
        # index_path = os.path.join(config.MANGA_VIEWER_ROOT_PATH, "manga_index.json")
        # idx = MangaIndex.load_index(index_path)
        db_folder_index = Repository.manga_index.folders.get(folder_id)
        if not db_folder_index:
            return

        # update name and path
        if new_folder_instance.name and new_folder_instance.name != db_folder_index.name:
    ...
    # @staticmethod
    # def is_exist(folder_id: str):
    #     index_path = os.path.join(config.MANGA_VIEWER_ROOT_PATH, "manga_index.json")
    #     idx = MangaIndex.load_index(index_path)
    #     return folder_id in idx.folders

    # @staticmethod
    # def find_by_folder_name(name: str):
    #     index_path = os.path.join(config.MANGA_VIEWER_ROOT_PATH, "manga_index.json")
    #     idx = MangaIndex.load_index(index_path)
